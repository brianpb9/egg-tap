...
- **V3.5 (fun + monetisasi)** ✅ Engagement: first-hatch=Bubu, Wish Star (50⭐→telur epic gratis), cadence challenge dinamis (toggle Parent Zone), combo benar-beruntun, Daily Surprise egg (dijamin baru), Favorite Buddy (sambut di title), perayaan dunia baru + finale 100%, BGM asli per-dunia (`assets/audio/bgm/*.mp3` via `BGMFILE`/`BGMTITLE`). Variety: tema belajar per-dunia (`WORLD_FLAVOR`), Ocean = challenge bubble. **Umur 5**: hanya tier 3–4 (mudah dibuang, floor 3) + mode asesmen (salah=skip). A11y: aria-label, reduced-motion, kontras AA, Musik/Efek terpisah, focus-visible, broken-img resilience.
- **Monetisasi (AdMob freemium)** ✅ Web-side siap: `ADMOB`(ID placeholder)/`ADS`(child-directed, no-op di web)/`removeAds`/`grantAdFree`/`restorePurchases`; interstitial tiap `AD_EVERY=2` hatch via `ADS.interstitial()` di `nextEgg` (no-op tanpa plugin). Tombol "Remove Ads"+Restore di Parent Zone (`S.adsRemoved`). `manifest.json` (PWA). **Build Android+AdMob+Billing → lihat `ADMOB_SETUP.md`** (wajib Families Ads Program + child-directed). Native build/akun/ID = tugas user.
- Audit ~**9.4/10**. Sisa ke 10/10: (1) playtest anak nyata (rasa fun, hanya tervalidasi manusia); (2) build native + submit Play Store; (3) animasi karakter (Spine) untuk produksi.
